--[[
    Pulled from https://gist.github.com/RyanPattison/7dd900f4042e8a6f9f23 as Public Domain
    A Lua implementation of the Alias Method, for sampling from an arbitrary distribution
    Slightly modified for purpose & to make use of Isaac's RNG class
    Use time is (O)1, preprocessing time is (O)N
]]--
...
function alias_table:new(weights)
  local total = 0
  for i=1, #weights do
      assert(weights[i] >= 0, "all weights must be non-negative")
...
alias_table = include("alias_table)
rng = RNG()
